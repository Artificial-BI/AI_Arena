...
import os
from midas.dpt_depth import DPTDepthModel  # Импортируем модель
...
# Создаём объект модели (нужно указать правильные параметры)
model = DPTDepthModel(
    path=model_path,
    backbone="beitl16_384",  # используемая архитектура
    non_negative=True
)
...
device = torch.device("cpu")  # Явно указываем на использование CPU
model.to(device)
model.eval()
